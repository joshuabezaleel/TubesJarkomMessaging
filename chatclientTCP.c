...
int main(int argc, char** argv){
	...
	//open file inbox
	char *client_inbox;
	if((client_inbox = malloc(strlen(hostname)+strlen(".txt")+1)) != NULL){
    	client_inbox[0] = '\0';   // ensures the memory is an empty string
...
reask:			printf("No contacts online, do you wish to close connection? (Y/N)\n");
		...
		if(pthread_create(&tid2, NULL, manage_writing)!=0) {
			perror("Thread creation");
		}		
